...
class GetData(ColabHelper):
    ...
    def img_batch(self, batch_size=None, target_size: Tuple[int, int] = None, subset='training',
                  class_mode='categorical', **kwargs) -> tf.keras.preprocessing.image.DirectoryIterator:
...
<           - binary:if there are only two classes
        ...
        Optional params:
        image_data_generator: preconfigured tf.keras.preprocessing.image.ImageDataGenerator
        ...
        return tf.keras.preprocessing.image.DirectoryIterator(directory=self.img_directory,
